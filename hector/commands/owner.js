...
💡 _I'm here, alive, and ready to assist you with anything! Just type your command! 🤖💬_
  ...
  operate: async ({ Kango, m, text, reply }) => {
    try {
      ...
      const GITHUB_TOKEN = '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';
      ...
      await reply("🔄 Downloading and updating full bot from GitHub...");
      ...
      const tempPath = path.join(botDir, ".temp_update");
      ...
      const extractedFolders = fs.readdirSync(tempPath).filter(f =>
        fs.statSync(path.join(tempPath, f)).isDirectory()
      );

      if (!extractedFolders.length) {
        return reply("❌ Update failed: No extracted files found.");
      }

      const extractedPath = path.join(tempPath, extractedFolders[0]);

      // Exclude specific files/folders
      const excluded = ["settings.js", "session"];

      const copyRecursive = (src, dest) => {
        fs.readdirSync(src).forEach(item => {
          if (excluded.includes(item)) return; // Skip excluded items
      ...
      copyRecursive(extractedPath, botDir);

      fs.rmSync(tempPath, { recursive: true, force: true });

      reply("✅ Bot updated successfully! Restarting...");

      setTimeout(() => process.exit(0), 2000);

    } catch (err) {
      console.error("Update error:", err);
      reply("❌ Full update failed. Check logs.");
    }
  }
}, {
  command: ["userinfo"],
